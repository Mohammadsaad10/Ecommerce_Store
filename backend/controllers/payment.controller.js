...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    console.log("creating checkout session, total amount:", totalAmount);
    ...
    // NOTE: `totalAmount` is in cents, so compare against 200 * 100 = 20000
    if (totalAmount >= 200 * 100) {
      await createNewCoupon(req.user._id);
      console.log(
        "New coupon created for user:",
        req.user._id,
        "(total in cents)",
        totalAmount
      );
    ...
    // Return the ID, URL and the total amount in dollars
    // Stripe now provides a `url` on the session which should be used for client redirects.
    res.status(200).json({
      id: session.id,
      url: session.url,
...
export const checkoutSuccess = async (req, res) => {
  try {
    ...
    if (session.payment_status === "paid") {
      // Idempotency: if an order for this Stripe session already exists, return it
      ...
      try {
        ...
        // Handle rare race condition where another process inserted the order concurrently
        if (
          err.code === 11000 &&
          err.keyValue &&
          err.keyValue.stripeSessionId
        ) {
          const already = await Order.findOne({ stripeSessionId: sessionId });
          if (already) {
            return res.status(200).json({
              success: true,
...
// IMP : we need to change the client url in success_url and cancel_url to production url when deploying the app to production.
