...
export const getAnalyticsData = async () => {
    ...
    const {totalSales, totalRevenue} = salesData[0] || {totalSales: 0, totalRevenue: 0}; //as _id: null , only one document will be returned. Thus, this line says if salesData[0] exists, take it or if it doesn't exist, take {totalSales: 0, totalRevenue: 0}
...
export const getDailySalesData = async (startDate, endDate) => {
    try {
        ...
        const dailySalesData = await Order.aggregate([
            {
                $match: { //$match => filters the documents in the collection based on the specified condition.
                    createdAt: {
                        $gte: startDate,
                        $lte: endDate,
                    }
            ...
            {
                $group: { //$group => groups the documents in the collection based on the specified condition.
